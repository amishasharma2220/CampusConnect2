...
const EventsSection = () => {
  ...
  useEffect(() => {
    eventsApi.getAll().then(data => setEvents(data.slice(0, 6))).catch(() => {});
    ...
    <section id="events" className="py-24 bg-muted/50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <div>
            <span className="text-sm font-semibold text-primary uppercase tracking-wider">What's Happening</span>
            <h2 className="font-display text-4xl sm:text-5xl font-bold text-foreground mt-3">Upcoming Events</h2>
          </div>
          <Button variant="hero" size="lg" className="mt-6 sm:mt-0" asChild>
            <Link to="/events">View All Events <ArrowRight className="w-4 h-4" /></Link>
          </Button>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map((event, index) => {
            ...
            const color = event.color || "from-primary to-accent";
            ...
            return (
              <motion.div key={event.id}
                ...
                <Link to={`/events/${event.slug}`} className="absolute inset-0 z-10" />
                <div className={`h-1.5 bg-gradient-to-r ${color}`} />
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                  <div className={`absolute -top-20 -right-20 w-40 h-40 rounded-full bg-gradient-to-br ${color} opacity-10 blur-3xl`} />
                </div>
                ...
                <div className="relative p-6 sm:p-8">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-accent/10 text-accent text-xs font-semibold">
                        <Flame className="w-3 h-3" />{countdown}
                      </span>
                    </div>
                    <span className="flex items-center gap-1 text-sm text-muted-foreground">
                      <Users className="w-3.5 h-3.5" />{event.registration_count.toLocaleString()}
                    </span>
                  </div>
                  ...
                  <div className="mb-5">
                    ...
                    {event.organizer_club && (
                      <p className="text-[11px] text-muted-foreground mt-1">By {event.organizer_club}</p>
                    )}
                  </div>
